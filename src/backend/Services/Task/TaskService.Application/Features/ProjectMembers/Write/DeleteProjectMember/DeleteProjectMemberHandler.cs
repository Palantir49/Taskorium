...
public class DeleteProjectMemberHandler(TaskServiceDbContext context, HybridCache cache)
    : IRequestHandler<DeleteProjectMemberCommand, int>
{
    public async Task<int> Handle(DeleteProjectMemberCommand request, CancellationToken cancellationToken = default)
    {
        var member = await context.ProjectMembers
                         .Include(m => m.User)
                         .FirstOrDefaultAsync(x => x.UserId == request.UserId && x.ProjectId == request.ProjectId,
                             cancellationToken)
        ...
        var deleteCount = await context.SaveChangesAsync(cancellationToken);
