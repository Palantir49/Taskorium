...
public class WorkspaceService(IWorkspaceRepository workspaceRepository) : IWorkspaceService
{
        ...
        CancellationToken ct = default)
    {
        ...
        return new WorkspaceResponse(issue.Id, issue.Name.ToString(), issue.CreatedDate, issue.OwnerId);
    }
    ...
    public async Task<List<WorkspaceResponse>> GetByOwnerIdAsync(Guid ownerId, CancellationToken ct = default)
    {
        ...
        return result.Select(w => new WorkspaceResponse(w.Id, w.Name.ToString(), w.CreatedDate, w.OwnerId)).ToList();
    }
    ...
    public async Task<WorkspaceResponse?> GetWorkspaceByIdAsync(Guid id, CancellationToken ct = default)
    {
        ...
        return new WorkspaceResponse(workspace.Id, workspace.Name.ToString(), workspace.CreatedDate, workspace.OwnerId);
    }
}
