...
import {IssueCreatedPayload} from "../types/notification.ts";
    ...
    switch (action.type) {
        ...
        case ActionTypes.SET_FILTER:
            return {
                ...
                filters: {
                    ...state.filters,
                    [(action.payload as { key: string; value: string }).key]: (action.payload as {
                        key: string;
                        value: string
                    }).value
                }
...
export function TaskProvider({children, projectId}: TaskProviderProps) {
    ...
    useEffect(() => {
        ...
        if (!latest) return;
        if (latest.id === lastHandledId.current) return;        // уже обработали
        if (latest.eventType !== 'IssueCreated') return;

        const payload = latest.payload as IssueCreatedPayload;
        if (!payload?.issueId) return;
        if (payload.projectId !== projectId) return;            // чужой проект

        lastHandledId.current = latest.id;

        const exists = state.tasks.find(t => t.id === payload.issueId);
        if (!exists) {
            fetchTaskById(payload.issueId).then(task => {
